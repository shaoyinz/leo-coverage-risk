...
INGESTION_AGENT = AgentDefinition(
    ...
    prompt=(
        ...
        "asset_href. For off-catalog factors (canopy height, buildings), use the manifest's "
        "source_url directly.\n"
        "3. Call fetch_aligned_surface(tile_id, bbox, manifest={surface/terrain/canopy/buildings "
        ...
        "tile; else fuse a pseudo-DSM from DEM + max(canopy, building). Buildings come from "
        "OpenBuildingMap and fuse opportunistically into the mosaic/pseudo_dsm fill (the "
        "non-lidar regions); a failed OBM read just drops the building term and keeps the tile.\n"
...
DATA_DISCOVERY_AGENT = AgentDefinition(
    ...
    prompt=(
        ...
        "buildings with heights — prefer OpenBuildingMap, which carries per-building height/"
        "storeys on a quadkey GeoParquet grid, with Overture as an alternative). Only fall "
        "back to WebSearch/WebFetch to fill a gap "
...
GEO_ANALYSIS_AGENT = AgentDefinition(
    ...
    prompt=(
        ...
        "roof-only default (0 m) first; for any at_risk/severe location, re-score with a higher "
        "dish_height_m (e.g. 0.3 m then 0.6 m — the Starlink 1–2 ft poles) so you can report "
        "'clear if raised to X m' rather than a flat fail.\n"
