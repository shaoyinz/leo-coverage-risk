...
> **What this repo implements.** The code ships a 5-agent proof-of-concept — **`data-discovery`** (A1; live STAC catalog + web search → ranked data manifest), **`ingestion`** (A2; live per-tile COG fetch + align → aligned pseudo-DSM), `geo-analysis` (A3; per-location obstruction sampling + risk scoring), **`qa`** (A4; live input-quality audit + output anomaly checks), and **`reporting`** (A5; live county/state aggregates + interactive PMTiles map + officer decision log) — defined in [`src/leo_pipeline/agents`](../src/leo_pipeline/agents/__init__.py). Input profiling + coordinate de-dup and UTM tiling are deterministic pre-steps ([`ingest.py`](../src/leo_pipeline/ingest.py) / [`tiling.py`](../src/leo_pipeline/tiling.py)), not an agent. **A1 is live:** `get_aoi_bbox`, `stac_search` (pystac-client over Microsoft Planetary Computer), and `write_data_manifest` return real results and persist the manifest the H1 gate reviews; driven on its own via `python -m leo_pipeline.run_discovery` (with an optional `--bbox` AOI override, against which `get_aoi_bbox` counts the locations actually inside the box). **A2 is live:** `stac_item_read` (resolve an approved collection to a signed, download-ready COG for a tile), `fetch_aligned_surface` (windowed rasterio read → reproject to the tile's auto-UTM zone → composite a **best-available-per-pixel mosaic**: lidar DSM where present, the globally complete DEM (+canopy) filling the holes, with a per-pixel **provenance** band; tile-keyed, content-addressed, idempotent cache), and `cache_rw` ([`src/leo_pipeline/tools`](../src/leo_pipeline/tools/__init__.py)); driven on its own via `python -m leo_pipeline.run_ingestion`. Both are covered by an offline `tests/` suite (plus opt-in live tests). The mosaic is the fix for **partial lidar coverage** (lidar is project-based and patchy): without it, ~half a real tile's points fell to `undetermined`; with it they are scored on the DEM fill at low confidence. The non-lidar fill term is `DEM + max(canopy, building)`: **building height is fused from OpenBuildingMap** (per-building height rasterized onto the tile grid via `_rasterize_building_window`, tagged with the `structure` provenance code) so building obstruction is modelled where lidar is absent — a previously deferred gap, now closed. **A3 is live:** `compute_sky_obstruction` (per-azimuth horizon profile over the A2 surface → dwell-time-weighted obstruction % → `clear`/`at_risk`/`severe`/`undetermined` tier) and `find_clear_sky_spot` (scenario 3: grid + mast-height search for a lower-obstruction position), backed by the deterministic engine in [`horizon.py`](../src/leo_pipeline/horizon.py) and the version-stamped obstruction spec ([`config.Analysis`](../src/leo_pipeline/config.py)); driven on its own via `python -m leo_pipeline.run_analysis` (with a no-API `--compute` path that runs the same engine directly). Deferred in A3's live core: σ_H is folded into the confidence flag rather than a full probabilistic clearance-margin model, and `az_weighting='tle_derived'` (the dwell-time map) falls back to the static north-biased gradient. **A4 is live:** `qa_input_audit` (one DuckDB pass over the locations table sizing the quarantine buckets — null/out-of-range coords, the (0,0) null island, off-AOI points, and lat/lon-swapped rows) and `qa_location_batch` (output-anomaly rules over the A3 findings: a region implausibly saturated with at-risk locations — grouped by **tile**, and by **county** FIPS when the dedup maps are on disk, household-weighted by `n_locations` — plus risk-tier↔`obstruction_pct` inconsistency, undetermined/low-confidence coverage budgets, a non-zero degenerate distribution, and `spec_version` drift), backed by the deterministic engine in [`qa.py`](../src/leo_pipeline/qa.py) and the version-stamped QA spec ([`config.QA`](../src/leo_pipeline/config.py)); the rules detect, the LLM only triages and routes to the H2 gate. Driven on its own via `python -m leo_pipeline.run_qa` (with a no-API `--compute` path that runs the same engine directly). **A5 is live:** `aggregate_findings` (roll the A3 findings up to **county** + **state**, household-weighted by `n_locations` via the dedup map, ranked by at-risk households), `render_map` (a per-location GeoJSON point layer coloured by risk tier → **PMTiles** via `tippecanoe` → a self-contained **MapLibre** HTML viewer; scales to ~1M points, no server — the bonus interactive map), and `write_report` (a scoped write of the officer-facing decision log into `outputs/`), backed by the deterministic engine in [`report.py`](../src/leo_pipeline/report.py) and the version-stamped [`config.Reporting`](../src/leo_pipeline/config.py); the aggregation + map tiles are deterministic, the LLM only writes the narrative. Driven on its own via `python -m leo_pipeline.run_report` (with a no-API `--compute` path that runs the same engine directly). **All six roles of the design below are now wired into `src/`** (Orchestrator + A1–A5); the remaining gaps are the documented deferrals within roles (A2 cover-proxy, A3 σ_H/TLE weighting; building height is now fused from OpenBuildingMap), not missing roles.
...
{ "name": "fetch_aligned_surface",
  "description": "Read COG windows for a tile, reproject to a metric CRS, composite into a best-available-per-pixel mosaic: lidar DSM where present, DEM (+canopy) filling the holes. Cached & idempotent.",
  "input_schema": { "type":"object",
    "properties": {
      "tile_id": {"type":"string"},
      ...
      "surface_mode": {"type":"string","enum":["mosaic","true_dsm","pseudo_dsm","cover_proxy"]} },
    "required":["tile_id","bbox","manifest"] },
  "returns": "{dsm_uri, crs, gsd_m, surface_mode, provenance_band, provenance_fractions, vintage_map, coverage_flag, confidence, valid_fraction}" }
// shipped & live (src/leo_pipeline/tools): windowed rasterio read + reproject (auto-UTM) +
// numpy composite, writing a **2-band** tile-keyed content-addressed GeoTIFF cache — band 1
// elevation, band 2 per-pixel provenance (lidar / DEM+canopy / DEM+building / bare-DEM fill). `mosaic` is the
// default when the manifest carries both a lidar DSM and a DEM: lidar where it has data, DEM
// filling every hole, so coverage is complete and a *partial* lidar tile no longer leaves half
// its points `undetermined`. The provenance band drives the A3 confidence flag (a bare-DEM-fill
// point is scored but flagged low-confidence). On a layer read failure it returns a structured
// error naming the next surface_mode down the hierarchy, so the LLM's only call is which
// fallback to retry. A companion `stac_item_read` resolves an A1 collection to a signed COG href
// per tile, and `cache_rw` lets the agent skip already-built tiles. The non-lidar fill term is
// `DEM + max(canopy, building)`: building height comes from OpenBuildingMap (rasterized per tile,
// `structure` provenance) — previously a deferred gap, now fused.
...
{ "name": "compute_sky_obstruction",
  ...
  "returns": "array<{location_id, obstruction_pct, blocked_azimuths, horizon_profile, risk_tier, confidence, surface_provenance}>" }
// confidence is provenance-driven (lidar>DEM+canopy>bare-DEM fill) + near-field sampling
// coverage + a σ_H clearance-margin borderline test (config.Analysis); `undetermined` is now
// only a genuine no-datum point, not a lidar coverage gap (the A2 mosaic fills those).
...
| Download / read error | Retry w/ backoff → **fall back down the data hierarchy** (`mosaic` lidar-over-DEM → true DSM → DEM+canopy-height → cover proxy) → mark tile `degraded`, lower `confidence`. |
| Partial lidar coverage (project-based, patchy) | Not a failure: the `mosaic` surface fills the holes from the complete DEM and records per-pixel provenance, so points on the fill are **scored at low confidence**, not dropped to `undetermined`. |
