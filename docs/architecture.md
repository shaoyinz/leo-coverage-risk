...
// elevation, band 2 per-pixel provenance (lidar / DEM+canopy / bare-DEM fill). `mosaic` is the
...
// per tile, and `cache_rw` lets the agent skip already-built tiles. Deferred: building-footprint
// fusion (the DEM+canopy term is canopy-only for now).
