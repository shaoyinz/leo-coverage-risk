...
the documented in-role deferrals (A2 cover-proxy, A3 σ_H/TLE weighting), not missing roles
(building height is now fused from OpenBuildingMap). See the [decision log](#decision-log).
...
Household-weighted, **~3.6%** of North Carolina locations (≈169.9K households) are at-risk.
The risk concentrates in dense-canopy/built-up counties and the western Blue Ridge terrain —
the priority county is **Mecklenburg (FIPS 37119, ~26.1K at-risk households)**, with the
highest *rate* in **Gaston (37071, 9.7%)** and smaller mountain counties (e.g. Madison 37115
at 9.2%, Yancey 37199 at 8.6%).

> This run fuses **building heights** (OpenBuildingMap) into the non-lidar surface; that is
> why the at-risk share rose from a pre-building 0.7% to **3.6%** and a `severe` tier appeared —
> tall structures are now visible obstructions in the ~43% of the AOI lidar doesn't cover.

![Coverage-risk map](outputs/coverage_map.png)

**Read it as a prioritised verify-on-site list, not a service guarantee** — 30.9% of scores
are low-confidence (DEM-fill / pseudo-DSM surfaces dominate, above the QA's 30% budget so the
run is flagged provisional) and the 🟡/🔴 cut-points are calibration defaults that should be
tuned against the Starlink app on a labelled sample first.
...
tile with unchanged inputs is a no-op. **Building height is fused from OpenBuildingMap**
(per-building height rasterized onto the tile grid and added to the non-lidar fill, tagged with
the `structure` provenance code); a failed OBM read just drops the building term and keeps the
tile. Buildings are opportunistic — lidar already carries roofs, so they matter in the
non-lidar (mosaic-fill / pseudo-DSM) regions.
...
|------|----------|-----------|
| 2026-06-14 | **Make the building source fully manifest-driven + add a human manifest-edit CLI** | Closed a single-source-of-truth seam: building access (the OBM quadkey URL template + zoom + GeoParquet column names) was read from `config.Ingestion`, so the manifest only *gated* buildings while config *sourced* them — you couldn't point A2 at a different building dataset by editing the manifest alone. Now the buildings manifest entry carries an `access_params` block (`url_template` with a `{quadkey}` placeholder, `quadkey_zoom`, `geom`/`height`/`bbox` columns; new `_building_access` resolver in `tools/__init__.py`), A2 resolves the per-tile URLs from *that* (folded into the content-addressed cache key so a source swap re-builds), and config values become **defaults** for omitted keys only. Back-compat preserved: a plain template string or bare on-switch still works off the defaults. Added a new `DatasetCandidate.access_params` field so the persisted manifest carries it end-to-end, and `offline.build_surface` now injects the OBM entry as an `access_params` object (also carrying vintage). **Made the H1 gate actionable** with `leo-manifest` (`manifest_edit.py`): `list`/`add`/`remove`/`delete` manifest entries — the intended path for a human to introduce a dataset A1's grounded discovery couldn't surface (e.g. a building-height source absent from every STAC catalog + the AWS registry), with the same grounding rule applied to human adds (a selected `web` pick with no resolvable source is de-selected with a note). Added 10 offline tests (building-access resolution, manifest carry-through, fetch uses the manifest template, + the manifest-edit CLI); full suite **201 passed / 3 skipped**. Docs updated (DESIGN §9.3/§16/§20). |
| 2026-06-04 | **Run the full 5,176-tile no-agent pipeline with buildings fused** (the deferred next step) | Ran the deterministic A2+A3 engine (`leo-full --workers 8`, no LLM / no API key, `caffeinate`-wrapped) over **every** tile, rebuilding surfaces **with OpenBuildingMap** (new cache key → 5,176 cache-miss rebuilds, ~176 min), then `leo-qa --compute` + `leo-report --compute`. **2,000 tiles gained building height** (≤11.5% of pixels). Result over all **4,514,477** unique coords: **96.4% clear / 3.2% at-risk / 0.4% severe / 0 undetermined** — the building fusion lifted at-risk from the pre-building 0.7% to 3.6% and surfaced a `severe` tier (16,412), since tall structures are now visible in the ~43% non-lidar AOI. A4: 0/4.67M rows quarantined, **1 warn anomaly** (`low_confidence_rate` 30.9% > 30% budget → run flagged provisional), no critical. 5 tiles initially failed on transient Azure 502s and were mopped up by a resume run (0 failures remaining). Refreshed the Insights table, `coverage_map.png`, and `outputs/` artifacts. |
| 2026-06-04 | **Close the building-height gap** — fuse OpenBuildingMap into the non-lidar surface | Buildings were a documented deferred gap: the pseudo-DSM/mosaic fill was `DEM + max(canopy, 0)` only, so off-lidar (≈43% of the AOI by provenance) building obstruction was invisible — a false-clear risk near tall structures. Wired in **OpenBuildingMap** (GFZ/GEM, ODbL) as the building term: US-covered, per-building height/storeys (GEM taxonomy; height from GHSL Built-up 2023A), published as cloud-native **GeoParquet on a level-6 quadkey grid** at source.coop. A2 now resolves the covering quadkey file from the tile bbox, reads building polygons + the GEM `height` string anonymously over **HTTPS via DuckDB** (`spatial`+`httpfs`, bbox-pre-filtered on the parquet's own `bbox` struct — no new dep; `s3fs` is absent), parses height to metres (`HHT:` metres / `H:`,`HBET:`,`HAPP:` storeys × 3 m, clamped), and **rasterizes it onto the exact same `_dst_grid` as the DEM/canopy reads** (tallest-wins), so it composites pixel-for-pixel. Fusion happens at the single locus `_composite_surface` step 2 (serves **both** mosaic-fill and pseudo-DSM), tagged with a new per-pixel provenance code `structure`(4) so the building contribution is auditable end-to-end (A3 maps it to medium confidence, like canopy fill). Buildings are **opportunistic** — lidar already carries roofs, a failed OBM read drops only the building term — and `pseudo_dsm` now accepts buildings as a first-class obstruction layer (canopy no longer required). Added 13 offline tests (GEM parser, quadkey math, grid-parity rasterize, building composite + provenance, mosaic fuse, graceful-degrade, cache-key) + 1 live OBM read; full suite **190 passed / 3 skipped**. **Verified live**: real OBM buildings fused onto a real Copernicus DEM over uptown Charlotte → **15.8% of pixels** gained building height (code 4, mean +21.7 m, up to the skyline). Deferred next step: a full 5,176-tile `leo-full --rescore`. |
